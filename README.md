...
The React app is a single-page application, so Netlify must serve `index.html` for any
client-routed path. The repository includes `client/public/_redirects` with the rule

```
/* /index.html 200
```

which Netlify copies into the built site and prevents the "Page not found" error when
refreshing or directly opening deep links. When configuring your site in Netlify:

1. Set **Base directory** to `client`
2. Set **Build command** to `npm run build`
3. Set **Publish directory** to `dist`

After these settings are saved, new deploys will include the redirect rule and routes
will load correctly.
